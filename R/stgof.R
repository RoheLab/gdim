## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## R implement of step-wise goodness-of-fit (arXiv:2009.09177)
## By: Fan Chen
## 10/05/2020
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
z2Z <- function(z) {
  f <- as.factor(z)
  sparseMatrix(i = seq_along(f), j = as.numeric(f), x = 1, 
...
SCORE <- function(A, K, 
  ...
  label = kmeans(rb, K, iter.max = iter.max, nstart = nstart, 
                 algorithm = algorithm, trace = trace)$cluster
}
...
#' @references Jin, Jiashun. Fast community detection by SCORE. Ann. Statist. 43 (2015), no. 1, 57--89. doi:10.1214/14-AOS1265. \url{https://projecteuclid.org/euclid.aos/1416322036}
RQ <- function(A, label){
  stopifnot("The dimensions of `A` and `label` disagree." = 
              nrow(A) == length(label))
